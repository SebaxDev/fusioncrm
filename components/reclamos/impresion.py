...
def render_impresion_reclamos(df_reclamos, df_clientes, user):
    ...
    st.markdown('<h2 class="impresion-header">📨 Impresión de Reclamos</h2>', unsafe_allow_html=True)
    st.caption("Genera reportes en formato PDF para los técnicos de campo")
    ...
    try:
        ...
        _mostrar_estadisticas_rapidas(df_merged)
        ...
        with st.expander("⚙️ Configuración de impresión", expanded=True):
            ...
            with col1:
                solo_pendientes = st.checkbox(
                    ...
                    value=True,
                    help="Filtrar solo los reclamos que están pendientes de resolución"
                )
            with col2:
                incluir_usuario = st.checkbox(
                    "👤 Incluir mi nombre en el PDF",
                    value=True,
                    help="Agregar tu nombre como responsable del reporte"
                )
    ...
    # === SECCIÓN: Reporte Diario (PNG) ===
    st.markdown("---")
    st.markdown('<h3 class="impresion-header">📄 Reporte Diario (PNG)</h3>', unsafe_allow_html=True)
    st.caption("Genera una imagen resumen de los reclamos del día en formato PNG")
    ...
    with col_img:
        if st.button("🖼️ Generar imagen del día", use_container_width=True):
            with st.spinner("Generando reporte diario..."):
                try:
                    # Usar el dataframe que recibió el componente
                    img_buffer = generar_reporte_diario_imagen(df_reclamos)
                    fecha_hoy = ahora_argentina().strftime("%Y-%m-%d")
                    
                    # Mostrar vista previa
                    st.image(img_buffer, caption=f"Reporte Diario - {fecha_hoy}", use_column_width=True)
                    
                    st.download_button(
                        label="⬇️ Descargar Reporte Diario (PNG)",
                        data=img_buffer,
                    ...
                    result['message'] = f"Reporte diario generado para {fecha_hoy}"
                    
                except Exception as e:
                    st.error(f"❌ Error al generar el reporte diario: {str(e)}")
                    if DEBUG_MODE:
                        st.exception(e)

    return result

def _mostrar_estadisticas_rapidas(df_merged):
    """Muestra tarjetas con estadísticas rápidas de reclamos"""
    total_reclamos = len(df_merged)
    pendientes = len(df_merged[df_merged["Estado"].astype(str).str.strip().str.lower() == "pendiente"])
    en_curso = len(df_merged[df_merged["Estado"].astype(str).str.strip().str.lower() == "en curso"])
    
    # CORRECCIÓN: Esta línea tenía un error de sintaxis
    desconexiones = len(df_merged[
    ...
    with col1:
        st.markdown(f'<div class="card-impresion">📊 Total reclamos <span class="stats-badge">{total_reclamos}</span></div>', 
                   unsafe_allow_html=True)
    
    with col2:
        st.markdown(f'<div class="card-impresion">⏳ Pendientes <span class="stats-badge">{pendientes}</span></div>', 
                   unsafe_allow_html=True)
    
    with col3:
        st.markdown(f'<div class="card-impresion">🔧 En curso <span class="stats-badge">{en_curso}</span></div>', 
                   unsafe_allow_html=True)
    
    with col4:
        st.markdown(f'<div class="card-impresion">🔌 Desconexiones <span class="stats-badge">{desconexiones}</span></div>', 
                   unsafe_allow_html=True)
...
def _mostrar_reclamos_pendientes(df_merged):
    ...
    with st.expander("🕒 Reclamos pendientes de resolución", expanded=True):
        ...
        if not df_pendientes.empty:
            ...
            if st.button("📋 Copiar lista de pendientes", key="copiar_pendientes"):
                texto = "\n".join([
                    f"{row['Nº Cliente']} - {row['Nombre']} - Sector {row['Sector']} - {row['Tipo de reclamo']}"
                    for _, row in df_pendientes_display.iterrows()
                ])
                st.session_state.clipboard = texto
                st.success("Lista copiada al portapapeles")
        else:
...
def _generar_pdf_todos_pendientes(df_merged, usuario=None):
    ...
    st.caption("Genera un PDF con todos los reclamos pendientes, ordenados según tu preferencia")
    ...
    if st.button("📄 Generar PDF con TODOS los pendientes", key="pdf_todos_pendientes", use_container_width=True):
...
def _generar_pdf_por_tipo(df_merged, solo_pendientes, usuario=None):
    ...
    st.caption("Selecciona uno o más tipos de reclamo para generar un PDF específico")
    ...
    if st.button("📄 Generar PDF de reclamos por tipo", key="pdf_tipo", use_container_width=True):
        buffer = _crear_pdf_reclamos(
            reclamos_filtrados, 
...
def _generar_pdf_manual(df_merged, solo_pendientes, usuario=None):
    ...
    st.caption("Selecciona manualmente los reclamos específicos que deseas incluir en el PDF")
    ...
    if st.button("📄 Generar PDF con seleccionados", key="pdf_manual", use_container_width=True):
        buffer = _crear_pdf_reclamos(
...
def _generar_pdf_desconexiones(df_merged, usuario=None):
    ...
    st.caption("Genera un listado específico para las desconexiones programadas")
    ...
    if st.button("📄 Generar PDF de desconexiones", key="pdf_desconexiones", use_container_width=True):
        buffer = _crear_pdf_reclamos(
            df_desconexiones, 
...
def _generar_pdf_en_curso_por_tecnico(df_merged, usuario=None):
    ...
    st.caption("Genera un listado de reclamos en curso agrupados por técnico asignado")
    ...
    if st.button("📄 Generar PDF de reclamos en curso por técnico", key="pdf_en_curso_tecnico", use_container_width=True):
        from reportlab.pdfgen import canvas
        from reportlab.lib.pagesizes import A4
        import io
