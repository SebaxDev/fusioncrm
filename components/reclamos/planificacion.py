...
def render_planificacion_grupos(df_reclamos, sheet_reclamos, user):
    ...
    st.markdown('<h2 class="planificacion-header">📋 Planificación de Grupos de Trabajo</h2>', unsafe_allow_html=True)
    st.caption("Asigna reclamos pendientes a grupos de técnicos para optimizar el trabajo en campo")
    ...
    try:
        ...
        if modo_distribucion != "Manual":
            if st.button("⚙️ Distribuir reclamos automáticamente", use_container_width=True):
        ...
        if st.session_state.get("vista_simulacion"):
            ...
            for grupo, reclamos in st.session_state.simulacion_asignaciones.items():
                with st.expander(f"📦 {grupo} - {len(reclamos)} reclamos", expanded=True):
                    for rid in reclamos:
            ...
            col1, col2 = st.columns(2)
            if col1.button("💾 Confirmar y guardar esta asignación", use_container_width=True):
            ...
            if col2.button("❌ Descartar simulación", use_container_width=True):
                st.session_state.vista_simulacion = False
                st.rerun()
        ...
        if df_pendientes is not None:
            ...
            # Botón de refresco
            if st.button("🔄 Refrescar reclamos", use_container_width=True):
                st.cache_data.clear()
...
def _mostrar_reclamos_asignados(df_pendientes, grupos_activos):
    ...
    for grupo in GRUPOS_POSIBLES[:grupos_activos]:
        ...
        st.markdown(f'<div class="grupo-card"><h4>🔢 {grupo} - Técnicos: {", ".join(tecnicos) if tecnicos else "Sin asignar"} ({len(reclamos_ids)} reclamos)</h4>', unsafe_allow_html=True)
        ...
        if not reclamos_grupo.empty:
            ...
            st.markdown(f"**Tipos:** {resumen_tipos}")
        ...
        if materiales_total:
            ...
            for mat, cant in materiales_total.items():
                st.markdown(f'<span class="materiales-badge">{cant} {mat.replace("_", " ").title()}</span>', unsafe_allow_html=True)
